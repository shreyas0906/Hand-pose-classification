...
with mp_hands.Hands(
  ...
  while cap.isOpened():
    ...
    # filler_image = np.zeros((480, 640, 3), dtype='uint8')
    # filler_image[:,:,0] = gray_image
    # filler_image[:,:,1] = gray_image
    # filler_image[:,:,2] = gray_image
    print(f"filler image: {filler_image.shape}")
    ...
    if results.multi_hand_landmarks:
      # print(f" result: {results.multi_hand_landmarks}")
      for hand_landmarks in results.multi_hand_landmarks:
        for point in mp_hands.HandLandmark:
          ...
          middle_finger_mcp = np.array((int(hand_landmarks.landmark[point.MIDDLE_FINGER_MCP].x * image_height),
